...
export class JourneyExecutor {
  ...
  async startJourney(journeyId: string, patientContext: PatientContext): Promise<string> {
    ...
    const runId = await this.journeyRepository.createJourneyRun(
      journeyId,
      patientContext,
  ...
  private async executeJourneyAsync(
    runId: string,
    journey: Journey,
    patientContext: PatientContext
  ): Promise<void> {
    try {
      let currentNodeId: string | null = journey.start_node_id;
  ...
  private executeMessageNode(node: ActionNode, patientContext: PatientContext): string | null {
  ...
  private executeDelayNode(node: DelayNode, runId: string): Promise<string | null> {
  ...
  private executeConditionalNode(node: ConditionalNode, patientContext: PatientContext): string | null {
  ...
  async getJourneyRunStatus(runId: string): Promise<JourneyRun | null> {
    return this.journeyRepository.getJourneyRunById(runId);
  }

  async cancelJourneyRun(runId: string): Promise<void> {
